...
// Ensure the mount node (#container) exists before initializing
function startWhenReady() {
  const container = document.getElementById("container");
  if (!container) {
    // Try again on next frame until Banner mounts the container
  ...
  material = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      float sceneSDF(vec3 pos) {
        ...
        vec3 topLeftPos = screenToWorld(vec2(0.0, 0.0));
